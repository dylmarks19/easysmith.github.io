function renderContact() {
  ...
  el('contact-left').innerHTML = `
    ...
    <div class="ci"><div class="ci-icon"><i class="ti ti-mail"></i></div><div><div class="ci-label">Email</div><div class="ci-val"><a href="mailto:${C.contact.email}">${C.contact.email}</a> <button onclick="navigator.clipboard.writeText('${C.contact.email}').then(()=>{this.textContent='Copied!';setTimeout(()=>{this.textContent='Copy'},2000)})" style="margin-left:8px;background:none;border:1px solid var(--border);color:var(--text-muted);font-size:11px;padding:2px 8px;border-radius:var(--r);cursor:pointer;">Copy</button></div></div></div>
