...
// Tracks which Products-page line sections are collapsed. Starts with
// every line collapsed (same reasoning as render-home.js — less scrolling
// on load) rather than an empty Set. Uses its own "products-line-section-*"
// id namespace — deliberately distinct from Home's "line-section-*" ids,
// since both pages' sections exist in the DOM at the same time (only one
// page is visually shown at once via CSS) and sharing ids would make
// getElementById silently grab the wrong one.
const collapsedProductsLineSections = new Set(PRODUCT_LINES.map(line => line.id));

function getProductsCatalogueForLine(line) {
  return line?.cataloguePdf || C.cataloguePdf;
}

function getProductsCatalogueLabelForLine(line) {
  return line?.cataloguePdfLabel || C.cataloguePdfLabel;
}

function updateProductsCatalogueButton(line) {
  const dlBtn = el('catalogue-download-btn');
  if (!dlBtn) return;

  const pdf = getProductsCatalogueForLine(line);
  const label = getProductsCatalogueLabelForLine(line);
  dlBtn.href = pdf;
  dlBtn.setAttribute('download', `${label}.pdf`);
  dlBtn.setAttribute('title', label);
}
...
function toggleProductsLineSection(lineId) {
  ...
  const line = PRODUCT_LINES.find(entry => entry.id === lineId);
  if (line) updateProductsCatalogueButton(line);
}
...
function renderProducts() {
  ...
  host.innerHTML = PRODUCT_LINES.map(line => `
    <div class="line-section${collapsedProductsLineSections.has(line.id) ? ' collapsed' : ''}" id="products-line-section-${line.id}">
      ...
      <button class="line-section-head" type="button" onclick="toggleProductsLineSection('${line.id}'); trackEvent('product_line_toggle', { line_id: '${line.id}', source: 'products' });" aria-expanded="${!collapsedProductsLineSections.has(line.id)}" aria-controls="products-line-body-${line.id}">
  ...
  const initialLine = PRODUCT_LINES[0];
  if (initialLine) updateProductsCatalogueButton(initialLine);
