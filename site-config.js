 ...
 * ============================================================================
 * !! THIS IS THE MAIN FILE TO EDIT FOR MOST CHANGES !!
 *
 * This file is the single source of truth for ALL content on the website.
 * It is split into two parts:
 *
 *   1. SITE_CONFIG  — Company info, contact details, hero text, stats,
 *                     certifications, enquiry form options, etc.
 *
 *   2. CATEGORIES   — The full product catalogue. Each entry defines one
 *                     product category (e.g. "Single Mast — Swing Outrigger"),
 *                     including its image, description, and model spec table.
 *
 * HOW TO MAKE COMMON EDITS:
 *   - Change a phone number   → find the "contact" section below
 *   - Change business hours   → find "hoursWeekday" / "hoursSat"
 *   - Change hero headline    → find "hero" section
 *   - Add/remove a stat       → find "stats" array
 *   - Edit certifications     → find "certifications" array
 *   - Add a product category  → copy an existing entry in CATEGORIES
 *   - Add a product model     → add a row to the category's "tableRows" array
 *
 * ICON NAMES: Icons come from Tabler Icons (https://tabler.io/icons).
 *             Search for an icon name there and paste it as e.g. "ti-certificate".
 */


/* ============================================================================
   PART 1 — SITE_CONFIG
   General company and site settings. Editable by a non-developer.
   ============================================================================ */

const SITE_CONFIG = {

  // ── Company Identity ──────────────────────────────────────────────────────
  // The company name and branding shown in the footer and browser tab.
  company: {
  ...
  // ── Contact Details ───────────────────────────────────────────────────────
  // Phone numbers shown on the Contact page. Add more entries by copying
  // the block below and removing the "//" comment marks.
  // The "whatsapp" field must be a number in international format WITHOUT "+"
  // e.g. South Africa +27 → start the number with "27"
  contact: {
    phones: [
      // {
      //   label: "Ivan",
  ...
  // Displayed on the Contact page beneath business hours.
  leadTimes: {
    standard: "8–10 weeks",
    custom:   "10–14 weeks",
  ...
  // This is the unique ID for the Formspree form endpoint.
  // When a user submits the enquiry form, it gets sent to this address.
  // To change the receiving email address, log in at https://formspree.io
  // and update the form settings there — no code change needed.
  formspreeId: "xlgvzdvo",

  // ── Product Catalogue PDF ─────────────────────────────────────────────────
  // The filename of the PDF catalogue placed in the same folder as index.html.
  // To update the catalogue, replace the file and update the name here.
  ...
  hero: {
    eyebrow: "South Africa's Industrial Equipment Specialists", // Small text above headline
    line1:   "Precision Equipment.",                           // First line of big headline
    line2:   "Delivered.",                                     // Second line (shown in a different colour)
    sub:     "Atlas Industrial Systems supplies world-class lifting and access equipment across South Africa — made to order.",
  },

  // ── Stats Bar ─────────────────────────────────────────────────────────────
  // The strip of figures shown below the hero section (e.g. "11 Product Categories").
  // Each entry has a "num" (the big number/text) and "lbl" (the label below it).
  stats: [
  ...
  // ── Trust Badges ─────────────────────────────────────────────────────────
  // The small icon + text items shown beneath the hero CTA buttons.
  trustBadges: [
  ...
  // ── Footer Badges ─────────────────────────────────────────────────────────
  // Small text tags shown in the bottom-right of every page footer.
  footerBadges: ["ISO 9001:2015", "CE Certified", "SANS Compliant", "EN 280/SANS 16368"],

  // ── Enquiry Form — Enquiry Type Dropdown ──────────────────────────────────
  // Options shown in the "Enquiry Type" dropdown on the Contact/Enquiry form.
  // Add, remove, or rename options here as needed.
  enquiryTypes: [
  ...
  // ── Certifications Page ───────────────────────────────────────────────────
  // Each entry renders one card on the Certifications & Compliance page.
  // tagType: "green" = green badge, "orange" = orange badge
  certifications: [
...
/* ============================================================================
   PART 2 — CATEGORIES (Product Catalogue)
   Each entry is one product category shown on the Home page and Products page.

   HOW TO ADD A NEW CATEGORY:
   1. Copy one of the entries below (from the opening { to the closing },)
   2. Paste it at the end of the array, before the final ];
   3. Fill in the details: id, code, title, image, shortDesc, tableHeaders, tableRows

   FIELD REFERENCE:
     id           — A unique short code used internally. No spaces, lowercase. e.g. "smp1"
     icon         — Tabler icon name. Used if no image is set. e.g. "ti-building-arch"
     code         — The product series code shown on cards and the comparison table. e.g. "SMP1"
     color        — Background colour of the card thumbnail. Use a dark hex colour. e.g. "#12181f"
     price        — Starting price in ZAR (shown on modals). Use 0 to show "Priced on enquiry".
     image        — Filename of the product image (must be in the same folder). e.g. "AWP1-raised.png"
                    Run optimize_images.py after adding new images to compress them.
     title        — Full category title shown on cards and accordions.
     shortDesc    — One or two sentence description of the category.
     tableHeaders — Column headings for the model spec table. Keep these consistent within a category.
     tableRows    — One array per model. Values must match the order of tableHeaders.
   ============================================================================ */
...
const CATEGORIES = [
  ...
  },
  {
    ...
    shortDesc: "Self-propelled aluminium single mast. Driven, Can turn 360 degrees on its own axis, indoor/outdoor.",
